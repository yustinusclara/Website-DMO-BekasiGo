...
export async function mockSignInWithPassword({ email, password, remember }) {
  ...
  const isTargetAccount = targetEmail === 'yustinusclara' || targetEmail === 'yustinusclara@bekasigo.id' || targetEmail === 'yustinusclara@gmail.com'

  if (isTargetAccount) {
    if (password !== 'Clariztri30') {
      throw new AuthError('invalid-credentials', 'The email or password is incorrect.')
    }
    return { user: { email: 'yustinusclara@bekasigo.id', role: 'editor' }, session: { token: 'mock-token' } }
  }

  // Fallback for general demo accounts
  if (password === 'wrong') {
    throw new AuthError('invalid-credentials', 'The email or password is incorrect.')
  }
