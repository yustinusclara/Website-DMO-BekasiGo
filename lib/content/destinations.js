...
export function getRelatedDestinations(dest, limit = 4) {
  ...
  const destCoords = dest.coords ?? coordFor(dest.id)
  
  const scored = DESTINATIONS
    .filter((d) => d.id !== dest.id)
    .map((d) => {
      const dCoords = d.coords ?? coordFor(d.id)
      const dist = getDistance(destCoords, dCoords)
      
      // Calculate similarity score (lower is more related)
      let score = dist // Base score is distance in km
      
      if (d.category === dest.category) {
        score -= 5 // Category similarity bonus
      }
      if (d.district === dest.district) {
        score -= 3 // Geographical proximity district bonus
      }
  ...
  return scored.map(s => buildDetail(s.item)).slice(0, limit)
}
