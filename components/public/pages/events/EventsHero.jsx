...
import { EVENT_META, EVENTS, eventStatus } from '@/lib/content/events'

export default function EventsHero() {
  const { hero } = EVENT_META
  const upcoming = EVENTS.filter((e) => eventStatus(e) !== 'past').length
        ...
        <div className="mt-8 md:mt-10 grid gap-8 md:grid-cols-[1fr_auto] items-end">
          ...
          >
            <span className="text-[10.5px] uppercase tracking-[0.22em] text-bekasi-gold-400 inline-flex items-center gap-1.5">
              <CalendarDays className="h-3 w-3" /> On the calendar
            </span>
            <span className="mt-2 font-display text-4xl md:text-5xl leading-none">{upcoming}</span>
            <span className="mt-2 text-xs text-white/60">Upcoming events · updated daily</span>
          </motion.div>
        </div>
      </div>
    </section>
