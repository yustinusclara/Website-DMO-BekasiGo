...
function SaveActions({ plan }) {
  ...
  const doDownload = useCallback(() => {
    try {
      // Trigger a clean print dialog for print-friendly one-pager layout,
      // and also download the JSON representation as a backup file.
      ...
      toast.success('Itinerary downloaded! Opening print dialog for PDF...')
      setTimeout(() => {
        window.print()
        setSavedFlash(null)
      }, 1000)
    ...
    <div className="rounded-xl bg-white border border-bekasi-emerald-900/8 overflow-hidden">
      ...
      <div className="p-5 space-y-2">
        <ActionRow
          ...
          label={savedFlash === 'saved' ? 'Saved to your account' : (submitting ? 'Saving...' : 'Save to my account')}
          hint="Requires sign-in · lands with auth"
          onClick={handleSave}
          disabled={submitting}
        />
        <ActionRow
          icon={savedFlash === 'downloaded' ? Check : Download}
          label={savedFlash === 'downloaded' ? 'Downloaded' : 'Download as PDF'}
          hint="Requires sign-in · lands with auth"
          onClick={handleDownload}
        />
        <ActionRow
          icon={savedFlash === 'shared' ? Check : Share2}
          label={savedFlash === 'shared' ? 'Link copied' : 'Share public link'}
          hint="Read-only URL, no login"
          onClick={handleShare}
        />
      </div>
    </div>
...
function ActionRow({ icon: Icon, label, hint, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full text-left rounded-lg border border-bekasi-emerald-900/8 hover:border-bekasi-emerald-900/25 hover:bg-bekasi-cream px-3 py-3 flex items-center gap-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    >
