  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {/* Poster image paints immediately (Cloudinary Floating Smart City) */}
        {content.poster && (
          <img
            src={content.poster}
            alt="Bekasi Skyline Poster"
            className={cn(
              "absolute inset-0 h-full w-full object-cover transition-opacity duration-1000 ease-out z-10",
              videoPlaying ? "opacity-0 pointer-events-none" : "opacity-100 scale-105"
            )}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-bekasi-emerald-900/70 via-bekasi-emerald-900/30 to-bekasi-emerald-900 z-20" />
        {/* left content vignette */}
        <div className="absolute inset-0 bg-gradient-to-r from-bekasi-emerald-900/75 via-bekasi-emerald-900/10 to-transparent z-20" />
        ...
        />
        {/* subtle film grain */}
        <div aria-hidden className="absolute inset-0 bg-noise opacity-[0.07] mix-blend-overlay pointer-events-none z-20" />
      </div>

      {/* --- CENTER CONTENT --- */}
      <div className="relative z-30 h-full container flex flex-col justify-end pt-28 md:pt-32 pb-20 md:pb-28">
          ...
          >
            <Link href={content.primaryCta.href} className="w-full sm:w-auto">
              <button className="btn-primary btn-lg w-full sm:w-auto group">
                <Sparkles className="h-4 w-4" /> {content.primaryCta.label}
                <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
              </button>
            </Link>
            <Link href={content.secondaryCta.href} className="group inline-flex items-center gap-3 text-white/90 hover:text-white">
              ...
              <span className="text-sm font-medium">{content.secondaryCta.label}</span>
            </Link>
          </div>
        </div>
      ...
      <div aria-hidden className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-b from-transparent to-bekasi-emerald-900/80 pointer-events-none z-20" />
    </section>
  )
}
