...
export default function UtilityPreview({ data }) {
  const content = data || UTILITY;
  const [activeId, setActiveId] = useState(content.tabs[0].id)
  const active = content.tabs.find((t) => t.id === activeId) ?? content.tabs[0]

  const pillItems = content.tabs.map((t) => ({ id: t.id, label: t.label, icon: ICON_MAP[t.icon] }))
      ...
      <div className="relative container py-20 md:py-28">
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-end">
          ...
          >
            ...
            <p className="mt-4 body text-bekasi-ink/65 max-w-lg">{content.kicker}</p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <Link href={content.action.href}>
              <button className="btn-primary btn-md">
                <Sparkles className="h-4 w-4" /> {content.action.label}
        ...
        >
          ...
          <div className="px-5 md:px-8 py-4 border-t border-black/[0.05] bg-bekasi-cream/40 flex flex-wrap items-center justify-between gap-3 text-xs text-bekasi-ink/60">
            ...
            <Link href={content.action.href} className="inline-flex items-center gap-1.5 text-bekasi-emerald-900 font-medium">
