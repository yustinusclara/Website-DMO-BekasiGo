...
const FALLBACK_SIGNATURE = {
  tag: 'Discover Bekasi',
  words: ['Floating City', 'Heritage', 'Smart City', 'Culinary Row', 'Waterfront', 'Situs & Lakes']
};

export default function FloatingSignature({ data }) {
  const content = data || (typeof SIGNATURE !== "undefined" ? SIGNATURE : FALLBACK_SIGNATURE);
  const words = [...content.words, ...content.words] // duplicate for seamless marquee
  return (
    <section aria-label="City signature" className="relative py-16 md:py-24 bg-bekasi-cream overflow-hidden">
      <div className="container text-center mb-8">
        <span className="eyebrow eyebrow-dot text-bekasi-emerald-700">{content.tag}</span>
      </div>
