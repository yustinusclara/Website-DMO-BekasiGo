...
export default function QuickExplore() {
  const items = QUICK_EXPLORE.categories.map((c) => ({
    id: c.id,
    label: c.label,
  ...
  const [activeId, setActiveId] = useState(QUICK_EXPLORE.categories[0].id)
  const active = QUICK_EXPLORE.categories.find((c) => c.id === activeId) ?? QUICK_EXPLORE.categories[0]
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setActiveId((cur) => {
        const idx = QUICK_EXPLORE.categories.findIndex((c) => c.id === cur)
        const next = QUICK_EXPLORE.categories[(idx + 1) % QUICK_EXPLORE.categories.length]
        return next.id
      })
    }, 6000)
        ...
        <div className="relative rounded-3xl bg-white shadow-elevated border border-black/[0.04] overflow-hidden">
          ...
          <div className="px-6 md:px-8 py-5 md:py-6 border-b border-black/[0.04] bg-bekasi-cream/40">
            <div className="flex items-center justify-between gap-4">
              ...
              />
              <Link
                href={QUICK_EXPLORE.action.href}
              ...
              >
                {QUICK_EXPLORE.action.label} <ArrowUpRight className="h-4 w-4" />
              </Link>
            </div>
          </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="mono text-bekasi-ink/40">
                    0{QUICK_EXPLORE.categories.findIndex((c) => c.id === active.id) + 1} / {QUICK_EXPLORE.categories.length}
                  </div>
