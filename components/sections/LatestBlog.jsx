  ...
  return (
    <section id="blog" className="relative overflow-hidden bg-bekasi-cream">
      <div className="container py-24 md:py-32 lg:py-36">
        <ChapterMarker text={BLOG.chapter} variant="light" />
        ...
        <div className="mt-14 md:mt-20 grid lg:grid-cols-12 gap-8 lg:gap-16 items-end">
          ...
          >
            <span className="eyebrow eyebrow-dot text-bekasi-emerald-700">{BLOG.eyebrow}</span>
          ...
          >
            <p className="body-lg text-bekasi-ink/70 max-w-lg">{BLOG.kicker}</p>
            <div className="mt-4 flex items-center gap-3 text-xs text-bekasi-ink/50">
              <Rss className="h-3.5 w-3.5" />
              <span className="font-mono">{BLOG.totalPosts}</span> articles in the archive
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 md:mt-12 flex flex-wrap items-center justify-between gap-4">
          <PillNav
            ...
            size="md"
          />
          <Link href={BLOG.action.href} className="hidden md:inline-flex items-center gap-1.5 text-sm font-medium text-bekasi-emerald-900 hover:text-bekasi-emerald-700 whitespace-nowrap">
            {BLOG.action.label} <ArrowUpRight className="h-4 w-4" />
          </Link>
        </div>
        ...
        >
          <div className="lg:col-span-7">
            <BlogCard item={BLOG.featured} variant="featured" />
          </div>
          <div className="lg:col-span-5">
            <div className="flex items-center justify-between mb-4">
              <span className="text-eyebrow uppercase text-bekasi-ink/60">More reads</span>
              <span className="mono text-bekasi-ink/40">{BLOG.compact.length}</span>
            </div>
            <ul className="divide-y divide-black/[0.06]">
              {BLOG.compact.map((post) => (
                <li key={post.href}><BlogCard item={post} variant="compact" /></li>
              ))}
            </ul>
            <Link href={BLOG.action.href} className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-bekasi-emerald-900 hover:text-bekasi-emerald-700">
        ...
        <div className="mt-16 md:mt-20 rounded-3xl bg-white border border-black/[0.05] p-6 md:p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link href={BLOG.secondaryAction.href} className="btn-ghost btn-md text-bekasi-emerald-900">
              {BLOG.secondaryAction.label}
            </Link>
            <Link href={BLOG.action.href}>
              <button className="btn-primary btn-md">
                <Rss className="h-4 w-4" /> {BLOG.action.label}
