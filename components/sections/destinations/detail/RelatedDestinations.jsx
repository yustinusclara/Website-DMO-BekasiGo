...
import DestinationCard from '@/components/ds/DestinationCard'

export default function RelatedDestinations({ items, currentTitle }) {
      ...
      <div className="container py-16 md:py-24">
        <div className="flex flex-wrap items-end justify-between gap-6 mb-8 md:mb-12">
          <div>
            ...
            <h2 className="mt-4 font-display text-3xl md:text-4xl leading-tight tracking-tight text-bekasi-emerald-900">
              More places worth pairing with {currentTitle}.
            </h2>
          </div>
          <Link
            href="/destinations"
        ...
        <div className="grid gap-5 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((d, i) => (
            ...
            >
              <DestinationCard item={d} view="grid" />
            </motion.div>
          ))}
        </div>
      </div>
